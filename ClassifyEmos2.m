...
addpath('B:/Dropbox/Documents/Research/Schwartz_Center/HFBroadBandModulationsExperiment/EmotionScripts') % Desktop
% addpath('C:\Users\Benjamin\Dropbox\Documents\Research\Schwartz_Center\HFBroadBandModulationsExperiment\EmotionScripts')
DataInfo %this matlab file loads all subject info needed
%savedat = 'SpecCoModMoreFreqs'; fullpaths = newpaths;
savedat = 'SpecCoModNoOvrlap'; 
% fullpaths = newpaths;
...
%-----------------------------------
% str = ['load', basedir, 'ConfusionTable.mat'];
% eval(str)
% ConfusionTable{subj,All/Val/Aro}.Strategy{Fscore/RandSel,1}{IM}(16,16)
%-----------------------------------
% str = ['load', basedir, 'AccuracyMatrix.mat'];
% eval(str)
...
%-----------------------------------
%%Each one of these files being loaded contains a 'Table' variable
% strs = {
%     ['load', basedir, 'Top_All_Fscore.mat'],
%     ['load', basedir, 'Top_Val_Fscore.mat'],
%     ['load', basedir, 'Top_Aro_Fscore.mat']
% };
% eval(str)
...
%   subj = subjlist(sbj); %this is a subject number from a list of subjects
%   if ~isempty(TotalAccuracy{subj, 1})
%     r = find(Table(:, subj)); %find indices of nonzero elements in argument                                     
%     r = r(end); %r = last nonzero index in Table(:, subj)
%     q = ConfusionTable{subj, 1}.Strategy{1, 1}{r}; %Fscore order, All
%     for e = 1:size(q, 1) - 1
%       eacc(subj, e) = 100*(q(e,e)/q(e,end));
%     end;
%     meannumsampls(1, subj) = mean(q(:,end)); %custom function
%     subjacc(1, subj) = TotalAccuracy{subj, 1}.Strategy{1, 1}.All(r,1); %only Fscore, %what are strategies?
%     subjacc(2, subj) = r; %number of IMs for 'all'
%     
%     eval(strs{3})%load Arousal Fscores
%     r = find(Table(:, subj));
%     r = r(end); %last nonzero indices in Table(:, subj)
%     subjacc(3,subj) = TotalAccuracy{subj,3}.Strategy{1,1}.All(r,1); % Arousal
%     subjacc(4,subj) = r; %number of IMs for 'Aro' (arousal)
%     
%     eval(strs{2})%load Valence Fscores
%     r = find(Table(:,subj)); 
%     r = r(end); %last nonzero indices in Table(:, subj)
%     subjacc(5,subj) = TotalAccuracy{subj, 2}.Strategy{1, 1}.All(r, 1); %Valence
%     subjacc(6,subj) = r; %number of IMs for 'Val' (valence)
%   end;
% end;
% mean(subjacc(1, find(subjacc(1,:)))) %mean value of all 'total' Fscore values for every subject
% medacc = median(eacc, 2); %median of the first two columns/dims
% %[aa bb] = sort(subjacc(1,:));% accuracy 'overall'
% [a b] = sort(medacc); % mean or median accuracy across emotions
% nonz = find(medacc(b) > 0); %nonzero positive indices of the medians of accuracies 
% b = b(nonz);
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FIGURE 1 ***************************************
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% figure; 
% boxplot(eacc(b, :)', 'plotstyle', 'compact', 'colors', 'g');
% set(gca, 'xlim', [.5 35.5]);
% xlabel('Sorted subjects'); 
% ylabel('Accuracy (%)'); 
% hold on;
% 
% ph = plot([1:length(b)], subjacc(2,b), 'bx', 'markersize', 12); 
% set(gca, 'xlim', [.5 35.5]);
% 
% plot([1:length(b)], subjacc(1,b), 'r^', 'linewidth', 2, 'markersize', 10); 
% hold on; %all
% 
% plot([1:length(b)], subjacc(3,b), 'md', 'linewidth', 2, 'markersize', 10); 
% hold on; %arousal
% 
% plot([1:length(b)], subjacc(5,b), 'cs', 'linewidth', 2, 'markersize', 10); 
% hold on; %valence
% 
% %plot([1:length(b)], meannumsampls(b), 'cx', 'linewidth', 2, 'markersize', 10); 
% %hold on;
% 
% xlabel('Sorted Subjects');
% set(gca, 'xlim', [.5 length(b)+.5]); 
% set(gca, 'ylim', [-1 101]);
% textsc(classes{1}, 'title'); %Get function from Julie
% 
% str = ['print', basedir, 'Accuracy', classes{1}, 'Medians.jpg -djpeg']; %saves variable to system
% eval(str)
% 
% set(gcf, 'PaperOrientation', 'landscape');
% set(gcf, 'PaperPosition', [0.25 0.25 10.5 8]); 
% 
% str = ['print', basedir, 'Accuracy', classes{classtype}, 'Medians.eps -depsc -adobe -painters']; %saves var to system as color postscript
% eval(str)
%end;
...
% figure; 
% boxplot(eacc, 'colors', jet(15));
% str = ['print C:\Users\julie\Documents\Manuscripts\EmotionClassification\Accuracy', classes{classtype}, 'byEmos.eps -depsc -adobe -painters']; 
% eval(str)

% no statistical diffs by anova1

% FIGURE ?  -- confusion matrix %where does the classtype variable come
% from?
...
% %where do the numbers 35 and 15 come from in this analysis?
% for subj = 1:35
%     if ~isempty(TotalAccuracy{subj, 1})
%         x = TotalAccuracy{subj, classtype}.Strategy{ForRand, 1}.Single;
%         z = reshape(x(end,:,:),[1 size(x,2)*size(x,3)]);
%         for im = 1:size(x, 1) %im is short for ...
%             y = reshape(x(im, :, :), [1 size(x, 2)*size(x, 3)]);
%             [h p(1, im)] = ttest2(y, z); %ttest2 - Two-sample t-test with pooled or unpooled variance estimate.
%         end;
%         r = find(p(2:end) > .01); 
%         r = r(1); % last consec under .01
%         confsum = zeros(15,15);
%         for e = 1:15
...
%             corrmatch = allaccs(e, e); %in a confusion matrix/table, the diagonal is number of samples correctly classified 
...
% figure; 
% x = sum(subjconf, 3); 
% x(find(x < 3)) = 0; %zero out ... something.
% imagesc(x); 
% cbar;
...
%   figure; 
%   [dens,minmask,maxmask] = PlotDipoles('sources.set', fullpaths,dlist{cls},[],[],[],{'mrislices',[58:-20:-22],'mriview','top','geom',[2 3]},'bred',[],'on');% unmasked
...
load E:/data/common1/emotion/Classification/YuanPinEmoWeights1sec.mat eweights comment
% last column for each subj is indicator variable (1:15)
% addpath('/home/julie/MatlabScripts/libsvm-mat-2.9-1')
addpath('C:\Users\Benjamin\Documents\MATLAB\libsvm-3.21')
...
for nx = 1:length(eweights)    
    ...
    %AllCMD{nx} = CMD;
    CMD = AllCMD{nx};
    clear Fs %leaves this variable undefined.
    for im = 1:size(eweights{nx}, 2) - 1 % last is emo index
        [P, table] = anovan(eweights{nx}(:,im), {eweights{nx}(:, end)}, 'display', 'off');
        Fs(im, 1) = table{2, 6};
    end;
...
% comment = The variable ''eweights'' is a cell array with all 35 subjects.
% Each cell (subject) contains a matrix with rows equal to the number of 
% spectral windows (0% overlap of 1-sec windows) by number of ICs*number of 
% freq bins.The FIVE Freq bins are: theta(3-8 Hz), Alpha (8-13.5 Hz), 
% Low Beta(13.5-20 Hz), Hi Beta (20-35 Hz), and Gamma (>35 Hz). 
% The columns are arranged as the five freq bands for each IC, followed 
% by the 5 freq bands (in order) for the next IC. Each spectral estimate 
% was obtained by FFT (pwelch method). The last column of the matrix is an 
% indicator matrix with numbers between 1 and 15 indicating which of the 15 
% emotions that spectral window (row) was taken from.; 
