...
eeglab
rawdat = '/data/common2/emotion/mm78/mm78.bdf';
spath = '/data/common2/emotion/mm78/';
...
ALLEEG=[];EEG=[];
EEG = pop_loadset( 'ButtonOnly.set', '/data/common2/emotion/mm78/');
...
for k = 1:length(emos)
    ...
    EEG = pop_select( EEG, 'nopoint',[seltime size(EEG.data,2)] );
...
rawdat = '/data/common2/emotion/mm78/mm78.bdf';
spath = '/data/common2/emotion/mm78/';
elpfile = '/data/common2/emotion/mm78/mm78-256.elp';

%blocksize = 800;
%[EEG,setnum] = Read256(rawdat,[spath,elpfile],spath,'tb-254-',[spath,' Emotion:'],blocksize);
...
pack
%%%%  block 2   %%%%%%%%%
EEG = pop_readbdf(rawdat, [1001:899:1900] ,265,[135:77:212]);
EEG = pop_select( EEG, 'nochannel',[255:264] );
EEG.chanlocs=pop_chanedit(EEG.chanlocs,  'load',{elpfile , 'filetype', 'autodetect'});
if EEG.chanlocs(1).labels == 'Nz'
    EEG.chanlocs(1:3) = [];
...
pack
%%%%%%  block 3  %%%%%%%
EEG = pop_readbdf(rawdat, [1901:999:2900] ,265,[135:77:212]);
EEG = pop_select( EEG, 'nochannel',[255:264] );
EEG.chanlocs=pop_chanedit(EEG.chanlocs,  'load',{elpfile , 'filetype', 'autodetect'});
if EEG.chanlocs(1).labels == 'Nz'
    EEG.chanlocs(1:3) = [];
...
pack
%%%%%%  block 4  %%%%%%%
EEG = pop_readbdf(rawdat, [2901:479:3380] ,265,[135:77:212]);
EEG = pop_select( EEG, 'nochannel',[255:264] );
EEG.chanlocs=pop_chanedit(EEG.chanlocs,  'load',{elpfile , 'filetype', 'autodetect'});
if EEG.chanlocs(1).labels == 'Nz'
    EEG.chanlocs(1:3) = [];
...
pack
%%%%%%  block 5  %%%%%%%
EEG = pop_readbdf(rawdat, [3381:894:4275] ,265,[135:77:212]); 
EEG = pop_select( EEG, 'nochannel',[255:264] );
EEG.chanlocs=pop_chanedit(EEG.chanlocs,  'load',{elpfile , 'filetype', 'autodetect'});
if EEG.chanlocs(1).labels == 'Nz'
    EEG.chanlocs(1:3) = [];
...
ALLEEG=[];EEG=[]; clear ans
%%%%%%  block 6  %%%%%%%
EEG = pop_readbdf(rawdat, [4276:692:4968] ,265,[135:77:212]); 
EEG = pop_select( EEG, 'nochannel',[255:264] );
EEG.chanlocs=pop_chanedit(EEG.chanlocs,  'load',{elpfile , 'filetype', 'autodetect'});
if EEG.chanlocs(1).labels == 'Nz'
    EEG.chanlocs(1:3) = [];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 mm78  - ~26 yo female
data collected on  using 256 (265 including analog) Biosemi acquisition system
...
[ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
    ...
    [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
    EEG = pop_eegthresh(EEG,1,[1:size(EEG.data,1)] ,-450,450,EEG.xmin,EEG.xmax,0,1);
...
numframes =
      ...
      125440   +   104448  +    116736   +    68608   +   125440    +   98304
...
%  
/data/common/matlab/ica_linux2.4 < /data/common2/emotion/mm78/mmemoICA.sc



eeglab;  chan = 203;  spath = '/data/common2/emotion/mm78/';  sname = 'mm78';
...
subj = [1,3,4,6,9,12,14,15,16,20,26];
...
spath = '/data/common2/emotion/mm78/';
datsets = {'emo-1-203.set','emo-2-203.set','emo-3-203.set','emo-4-203.set','emo-5-203.set','emo-6-203.set'};
