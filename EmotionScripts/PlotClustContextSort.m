...
function [outimages,eachmemdiff,meddiffs] = PlotClustContextSort(stem,fullpaths,idxname,icidx,clabels,delqs,imgtype,imgval,alpha,ttl,savestring,winwidthfac)

    if ~exist('winwidthfac')
        winwidthfac = 100;
    elseif isempty(winwidthfac)
        winwidthfac = 100;
    end;
...
if strcmp(imgtype,'oldview')%------------------------------------------
    %sortcon = keepsort; % this is the sorted THEN avg'd data (withn subject, this is nearly identical to sorting all wts together)
    imgxwidth = size(sortcon,2)/winwidthfac;
    imgxadv = .5;
