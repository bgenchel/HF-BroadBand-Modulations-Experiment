...
savedat = 'ChanPwrBrainBackProj'; frqlim = [3 125];overlap = 1;nfreqs = 370;freqscale = 'quad';wsize = 1; chancomp= 'chan';
savedat = 'ChanPwrBrainMuscBackProj'; frqlim = [3 125];overlap = 1;nfreqs = 370;freqscale = 'quad';wsize = 1; chancomp= 'chan';
savedat = 'ChanPwrBrainMuscHeartBackProj'; frqlim = [3 125];overlap = 1;nfreqs = 370;freqscale = 'quad';wsize = 1; chancomp= 'chan';
...
for nxx = 1:length(subjlist)
  nx = subjlist(nxx);
  EEG = pop_loadset('filename','sources.set','filepath',fullpaths{nx});
  muscles{nx} = EEG.muscle;
  heartbeats{nx} = EEG.heart;
end;
tmplist = [];
for nxx = 1:length(subjlist) % for channel decomps with backproj
  nx=subjlist(nxx);
  CalcFFTonly(datset,fullpaths{nx},[],savedat,frqlim,freqscale,overlap,nfreqs,wsize,chancomp,[gdcomps{nx} muscles{nx} heartbeats{nx}]);
%end;
...
save /data/common1/emotion/YuanPinChanBrainMuscHeartBackProjPwr.mat eweights comment
end;
comment = 'The variable ''eweights'' is a cell array with all 32 subjects. This is the same as the channel power matrix except that only brain and/or muscle/heart ICs were back projected to all channels. Each cell (subject) contains a matrix with rows equal to the number of spectral windows (0% overlap of 1-sec windows) by number of Channels*number of freq bins.The FIVE Freq bins are: theta(3-8 Hz), Alpha (8-13.5 Hz), Low Beta(13.5-20 Hz), Hi Beta (20-35 Hz), and Gamma (>35 Hz). The columns are arranged as the five freq bands for each Channel, followed by the 5 freq bands (in order) for the next Channel. Each spectral estimate was obtained by FFT (pwelch method). The last column of the matrix is an indicator matrix with numbers between 1 and 15 indicating which of the 15 emotions that spectral window (row) was taken from.'; 
save /data/common1/emotion/YuanPinChanBrainMuscHeartBackProjPwr.mat eweights comment
save /data/common1/emotion/YuanPinChanBrainMuscBackProjPwr.mat eweights comment
save /data/common1/emotion/YuanPinChanBrainBackProjPwr.mat eweights comment
