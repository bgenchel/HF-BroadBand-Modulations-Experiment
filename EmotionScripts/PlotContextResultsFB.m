...
%
% PlotContextResultsFB(pathname,filename,wtsmat,sphmat,savename,freqs,times,cxtlim,pcersp,epcanum,pccxt,cpcanum,pcstot,channo,ctxsigs,ctxstd,whichtmps);
%
...
function PlotContextResultsFB(pathname, datatitle, savename,ctlim,erspmask,ctxsigs,ctxstd,whichtmps);
    ...
    if ~isempty(s.pcerspnum) & isempty(s.pccntxnum) % ersp is pca's, but context IS NOT   
        ...
        ph = textsc(['ERSP/Context decomposition: PCA of ERSP to',int2str(s.pcerspnum),' dims; scalefac ',num2str(scalefac)],'title'); set(ph,'fontsize',14);
