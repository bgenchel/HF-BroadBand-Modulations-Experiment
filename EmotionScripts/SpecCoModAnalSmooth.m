...
function [meanpwr,numtrials,numframes,freqs,keeptrack,rmepochs,dstrials, pcs, rowmeans] = SpecCoModAnalSmooth(datset,datpath,complist,savedat,frqlim,freqscale,pcfac,overlap,auxpath,nfreqs,pwrdecomp,dec,smth);
    
    thresh = 3000;% threshold for uV auto-rejection (normally 1000)
    ...
    for ds = 1:length(datset)
        ...
        for cp = 1:length(complist)
            ...
            dat = squeeze(EEG.icaact(complist(cp),:,:)*multfact); % added multfact back 12-6-05
%%%%%%%%%%%%%%%%  Begin decompostion of single epochs  %%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if length(pwrdecomp) < 3 & pwrdecomp == 0 % FFT using pwelch          
                ...
                hwin = hanning(size(dat,2))';
                fprintf('\nUsing hanning windowed data...\n');
                if cp == 1
                    % to get number of freqs:
                    [p f] = pwelch(dat(1,:,1).*hwin,EEG.srate,EEG.srate*3/4,EEG.srate*10,EEG.srate);  
                    pwr = zeros(size(dat,3),length(f),size(alldat,1));
                end;
                pwr1 = zeros(size(dat,3),length(f));
                for epoch = 1:size(dat,3)
                    %[pwr(epoch,:) frqs] = pwelch(double(dat(cp,:,epoch).*hwin),EEG.srate,EEG.srate*3/4,freqs,EEG.srate); 
                    [pwr1(epoch,:) frqs] = pwelch(dat(cp,:,epoch).*hwin,EEG.srate,EEG.srate*3/4,EEG.srate*10,EEG.srate);  % old method
                    pwr1(epoch,:) = 10*log10(pwr(epoch,:,cp));% need to convert to log for 'modulator' interpretation
                end;   
                if ~isempty(smth)
                    [pwr1,outx] = movav(pwr1,[],smth,1,[],[],[],0);
                end;
                pwr(:,:,cp) = pwr1;
            elseif length(pwrdecomp) > 2 % use wavelet decomp
               
                if size(dat,1) > winsize
                    ndiff = size(dat,1) - winsize;
                    rempnts1 = floor(ndiff/2);
                    rempnts2 = ceil(ndiff/2);
                    if rempnts2 == 1
                        dat = dat(1:end-rempnts2,:,:);
                    elseif rempnts2 > rempnts1
                        dat = dat(rempnts2:end,:);
                        dat = dat(1:end-rempnts2,:);
                    else                    
                        dat = dat(rempnts2:end,:);
                        dat = dat(1:end-(rempnts2-1),:);
                    end;
                elseif size(dat,1) < winsize
                    fprintf('\nError: Epoch length must be equal or greater to the winsize (%s)\n\n',int2str(winsize));return;
                end;
                pwr1 = wavelets * (dat.*repmat(hwin',[1 size(dat,2)])); 
        ...
        if ds == 1
            ...
            allpwr(end+1:end+size(pwr,1),:,:) = pwr;
        end;
        if ds == 1
            keeptrack(ds,:) = [1 size(1,3)];
        else
            keeptrack(ds,:) = [keeptrack(ds-1,2)+1 keeptrack(ds-1,2)+size(pwr,1)];
        end;
        clear dat pwr
    end;  
    pwr = allpwr;     clear allpwr
    
    % added for testing:
    if ~isempty(dec)
        randdels = randperm(size(pwr,1));
        randdels = randdels(1:round(size(pwr,1)/dec));
        pwr = pwr(randdels,:,:);
    end;
    ...
    floatwrite(pwr, [datpath,savedat,'RAW.fdt']);% save raw data for subsequent decomps
    ...
    numframes = size(pwr,2);
    smooth = smth;
    str = ['save ',datpath,savedat,'.mat meanpwr numrows numframes freqs freqscale keeptrack rmepochs dstrials  pcs rowmeans complist overlap datset eigfile smooth'];eval(str);    
