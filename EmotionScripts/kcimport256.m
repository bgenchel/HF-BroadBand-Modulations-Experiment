% procedures to import emotion data on kc66
...
eeglab
rawdat = '/data/common2/emotion/kc66/kc66.bdf';
spath = '/data/common2/emotion/kc66/';
...
pack
EEG = pop_readbdf(rawdat, [5001:509:5510] ,265,[]);
...
ALLEEG=[];EEG=[];
EEG = pop_loadset( 'ButtonOnly.set', spath);
...
rawdat = '/data/common2/emotion/kc66/kc66.bdf';
spath = '/data/common2/emotion/kc66/';
elpfile = '/data/common2/emotion/kc66/kc66-256.elp';

%%%%  block 1   %%%%%%%%%
EEG = pop_readbdf(rawdat, [1:712:713] ,265,[135:77:212]);
...
[ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
EEG = pop_eegfilt( EEG, 1, 0, [], [0]); % high pass first is best
...
pack
%%%%  block 2   %%%%%%%%%
EEG = pop_readbdf(rawdat, [714:805:1519] ,265,[135:77:212]);
...
pack
%%%%%%  block 3  %%%%%%%
EEG = pop_readbdf(rawdat, [1520:1050:2570] ,265,[135:77:212]);
...
pack
%%%%%%  block 4  %%%%%%%
EEG = pop_readbdf(rawdat, [2571:999:3570] ,265,[135:77:212]);
...
pack
%%%%%%  block 5  %%%%%%%
EEG = pop_readbdf(rawdat, [3571:919:4490] ,265,[135:77:212]); 
...
pack 
%%%%%%  block 6  %%%%%%%
EEG = pop_readbdf(rawdat, [4491:999:5490] ,265,[135:77:212]);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 kc66  - ~38 yo female
data collected on August 13,2004  using 256 (265 including analog) Biosemi acquisition system
...
remove = [24,45,48,104,112,135,143,163,181,202,207,211,213,223];
datset = {'emo-1-254.set','emo-2-254.set','emo-3-254.set','emo-4-254.set','emo-5-254.set','emo-6-254.set'};
datset2 = {'emo-1-240.set','emo-2-240.set','emo-3-240.set','emo-4-240.set','emo-5-240.set','emo-6-240.set'};
...
numframes =
      ...
      125952  +   148224   +   235776  +    218880  +    210432  +    234240
...
cat   emo2.fdt  emo3.fdt   emo4.fdt   emo5.fdt   > emoall.fdt
%  
/data/common/matlab/ica_linux2.4 < /data/common2/emotion/kc66/kcemoICA.sc
...
spath = '/data/common2/emotion/kc66/';
eeglab;  chans = 240;
EEG = pop_loadset( ['emo-1-',int2str(chans),'.set'], spath);
[ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG);
sph=floatread([spath,'sph',int2str(chans),'pc100.sph'],[chans chans]); 
wts=floatread([spath,'wts',int2str(chans),'pc100.wts'],[100 chans]); 
EEG.icaweights=wts;EEG.icasphere=sph;EEG.icawinv=[];
[ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
pop_topoplot(EEG,0, [1:36] , 'kc66 Emotion Components; PCA to 100 ',[6 6] ,0, 'electrodes', 'off', 'masksurf', 'on');
set(gcf,'PaperOrientation','landscape');
set(gcf,'PaperPosition',[0.25 0.25 10.5 8]);
print -Pcoloring -dpsc2 

subj = [2:4,6:13,15,17,21,24,26];
...
for c = 1:length(subj)
    ...
    title(int2str(cc)); set(gca,'xtick',[5:5:40]); set(gca,'xlim',[1 40]); set(gca,'xgrid','on');
    pl=pl+3;
end;
ph=textsc(['kc66 Emotion Component Spectra (PCA to 100) from ',int2str(chans),' chan data, set 1'],'title');
set(ph,'fontsize',14);
set(gcf,'PaperOrientation','landscape');
set(gcf,'PaperPosition',[0.25 0.25 10.5 8]);
...
datsets = {'emo-1-240.set','emo-2-240.set','emo-3-240.set','emo-4-240.set','emo-5-240.set','emo-6-240.set'};
spath = '/data/common2/emotion/kc66/';
