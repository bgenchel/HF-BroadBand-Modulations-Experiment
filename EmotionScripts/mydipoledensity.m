...
function [prob3d, prob3dneg, mri] = mydipoledensity(dipplotargs, varargin)

    % what I added (JO):
        % contingency for pos and negative weights (also added neg density output)
        % no change to 'distance' and 'entropy' options, just 'alldistance'
% TO DO: return in dipplot() the real 3-D location of dipoles (in posxyz)
%        FIX the dimension order here
    prob3dneg = [];   
    prob3d = []; mri = [];prob3dneg = [];
    if nargin < 1
        help dipoledensity
        return
    end
...
for index = 1:length(struct)
    dips = size(struct(index).eleccoord,1); % single or dual dipole
    for dip = 1:dips
        allx(count) = struct(index).eleccoord(dip,1);
        ally(count) = struct(index).eleccoord(dip,2);
        allz(count) = struct(index).eleccoord(dip,3);
        alli(count) = index;
        %allw(count) = g.weight(index)/dips; % div by 2 if dual dipole
        allw(count) = g.weight(index); % or not. I don't think they should be half. -JO
...
if ~isempty(find(g.weight < 0))
    prob3dneg = zeros(ceil(g.mri.dim/g.subsample));
end;
% compute voxel size
% ------------------
...
if ~strcmpi(g.method, 'alldistance') % only alldistance works with neg weights
    ...
    if ~isempty(find(g.weight < 0))
        tmpprobneg = zeros(1, size(InsidePoints,2));
    end;
    for i = 1:size(allx,2)
        ...
        if g.weight(i) >= 0
            ...
            tmpprobneg = tmpprobneg + g.weight(i)*exp(-alldists/(2*g.methodparam^2)); % 3-D gaussian smooth
            if any(isinf(tmpprobneg)), dfdsfa; end;
        end;
    ...
    if ~isempty(find(g.weight < 0))
        for i = 1:length(Inside)
            pnts = allinds(:,Inside(i));
            prob3dneg(pnts(1), pnts(2), pnts(3)) = tmpprobneg(i);
        end;
    end;
    
end;
fprintf('\n');
clear tmpprobneg tmpprob
% normalize for points inside and outside the volume
% --------------------------------------------------
if strcmpi(g.method, 'alldistance') 

    disp('Normalizing to dipole/cm^3');
    %totdip = size(allx,2);   % number of dipoles
    totdip = length(find(g.weight > 0));   % number of dipoles
    totval = sum(prob3d(:));  % total values in the head
    %voxvol;                  % volume o af a voxel in mm^3  % what is this for?
    prob3d = prob3d/totval*totdip/voxvol*1000; % time 1000 to get cubic centimeters
    prob3d = prob3d/g.nsessions; % what is this?
    if ~isempty(find(g.weight < 0))
        totdip = length(find(g.weight < 0));   % number of dipoles
        totval = abs(sum(prob3dneg(:)));  % abs makes density stay negative
        prob3dneg = prob3dneg/totval*totdip/voxvol*1000; % time 1000 to get cubic centimeters
        prob3dneg = prob3dneg/g.nsessions; % what is this?
    end;
end;

% resample matrix%%  this is why I made my own version, I don't want division by**************
% ----------------%%  subsample factor, but I want the other stuff done here
if g.subsample ~= 1
    prob3d = prob3d/g.subsample; % do I need this??
    ...
    prob3d = newprob3d; clear newprob3d
    if ~isempty(find(g.weight < 0))
        prob3dneg = prob3dneg/g.subsample; % do I need this??
        newprob3dneg = zeros(g.mri.dim);
        X = ceil(g.mri.xgrid/g.subsample);
        Y = ceil(g.mri.ygrid/g.subsample);
        Z = ceil(g.mri.zgrid/g.subsample);
        for index = 1:size(newprob3dneg,3)
            newprob3dneg(:,:,index) = prob3dneg(X,Y,Z(index));
        end;    
        prob3dneg = newprob3dneg;clear newprob3dneg
    end;
end;
...
if g.smooth ~= 0
    ...
    if ~isempty(find(g.weight < 0))
        prob3dneg = smooth3d(prob3dneg, g.smooth);
    end;
end;
...
if strcmpi(g.plot, 'off')
    ...
    if ~isempty(find(g.weight < 0))
        mri3dplot( prob3dneg, g.mri, g.plotargs{:}); % plot the density using mri3dplot()
    end;
end;
return;
