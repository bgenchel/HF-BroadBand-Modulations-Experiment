% procedures to import emotion data on an70
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%  *** Import Button Only   ***  %%%%%%%%%%%%%%%%%%%%%%%%%%%
EEG = pop_readbdf(rawdat, [300:900:1200] ,265,[]);
...
eeglab
rawdat = '/data/common2/emotion/an70/An70.bdf';
spath = '/data/common2/emotion/an70/';
...
pack
EEG = pop_readbdf(rawdat, [4001:191:4192] ,265,[]);
...
end;  % ** check to see they are all non-zero!!!!
...
EEG = pop_loadset( 'ButtonOnly.set', '/data/common2/emotion/an70/');
...
rawdat = '/data/common2/emotion/an70/An70.bdf';
spath = '/data/common2/emotion/an70/';
elpfile = '/data/common2/emotion/an70/an70-256.elp';

EEG = pop_readbdf(rawdat, [1:929:930] ,265,[135:77:212]);
...
pack
%%%%  block 2   %%%%%%%%%
EEG = pop_readbdf(rawdat, [1400:980:2380] ,265,[135:77:212]);
...
pack
%%%%%%  block 3  %%%%%%%
EEG = pop_readbdf(rawdat, [2381:969:3350] ,265,[135:77:212]);
...
pack
%%%%%%  block 4  %%%%%%%
EEG = pop_readbdf(rawdat, [3351:649:4000] ,265,[135:77:212]);
...
an70  - 33 yo male
data collected on June 3, 2004  using 256 (265 including analog) Biosemi acquisition system
...
Performed No Button Emotion task # 5

eeglab
EEG = pop_loadset( 'emo-1-254.set', '/data/common2/emotion/an70/');
[ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG);

% Look at data and find bad channels
remove = [26,57,81,86];
datset = {'emo-1-254.set','emo-2-254.set','emo-3-254.set','emo-4-254.set'};
datset2 = {'emo-1-250.set','emo-2-250.set','emo-3-250.set','emo-4-250.set'};
floatset = {'/data/common2/emotion/an70/emo1.fdt','/data/common2/emotion/an70/emo2.fdt','/data/common2/emotion/an70/emo3.fdt','/data/common2/emotion/an70/emo4.fdt','/data/common2/emotion/an70/emo5.fdt'};
spath = '/data/common2/emotion/an70/';
...
% use pca  to 130 bc not enough data
/data/common/matlab/ica_linux2.4 < /data/common2/emotion/an70/anemoICA250.sc
...
eeglab
EEG = pop_loadset( 'emo-1-250.set', '/data/common2/emotion/an70/');
[ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG);

sph=floatread('/data/common2/emotion/an70/sph250pc100.sph',[250 250]); 
wts=floatread('/data/common2/emotion/an70/wts250pc100.wts',[100 250]); 
EEG.icaweights=wts;
EEG.icasphere=sph;
EEG.icawinv=[];
[ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
pop_topoplot(EEG,0, [1:36] , 'an70 Emotion Components; PCA to 100 ',[6 6] ,0, 'electrodes', 'off', 'masksurf', 'on');
, 'plotrad',0.5
set(gcf,'PaperOrientation','landscape');
set(gcf,'PaperPosition',[0.25 0.25 10.5 8]);
print -dpsc2 [-painters]

subj = [1:7,9,11,12,13,14,15,16,18,20,21,23,25,37,38,39,40];
...
ph=textsc('an70 Emotion Component Spectra (PCA to 100) from 250 chan data, set 1','title');
set(ph,'fontsize',14);
set(gcf,'PaperOrientation','landscape');
set(gcf,'PaperPosition',[0.25 0.25 10.5 8]);
...
spath = '/data/common2/emotion/an70/';
datsets = {'emo-1-250.set','emo-2-250.set','emo-3-250.set','emo-4-250.set'};
...
for ds = 1:length(datsets)
    ...
    for emo = 1:length(emos)-1:length(emos)
        if ~isempty(find(ismember({EEG.event.type}, emos{emo})))
            stf = EEG.event(find(ismember({EEG.event.type}, emos{emo}))).latency;
            endf = EEG.event(find(ismember({EEG.event.type}, emos{emo}))+1).latency;
            endt = endf/256;  stt = stf/256;  eptime = endt-stt;
