...
%
% [allcross, crossidx]] = PlotCrossCoh(datset,savename,fullpath,plotcomps,mask,cond)
%
%
% INPUTS:
...
function [allcross, crossidx] = PlotCrossCoh(datset,savename,fullpath,plotcomps,mask,cond)
    ...
    if cond > 0
        for cp1 = 1:length(plotcomps)-1
            ...
            for cp2 = cp1+1:length(plotcomps)
                onecross = s.cohercell{find(plotcomps(cp1)==s.complist),find(plotcomps(cp2)==s.complist),cond};
                if mask == 1
        ...
        for cp1 = 1:length(plotcomps)-1
            xx = 0;
            for cp2 = cp1+1:length(plotcomps)
                onecross = s.cohercell{find(plotcomps(cp1)==s.complist),find(plotcomps(cp2)==s.complist)};
                allcross(:,:,end+1) = onecross;
                crossidx(end+1,:) = [plotcomps(cp1) plotcomps(cp2)];
                if mask == 1
    ...
    textsc(['Cross Coherences for ',fullpath(end-4:end-1),' Condition ',int2str(cond),' from ',savename],'title');
    axcopy
