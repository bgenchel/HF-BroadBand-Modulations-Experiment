% finds mean spectrum for each emotion (and each subject) and compares
...
eeglab
%%%%%%%%% END VARIABLES %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  PLOTS EACH SBUJECT AS A FIGURE WITH SPECTRAL DIFFS FOR EACH EMO/COMP (MASKED FOR HI SIGNIFICANCE)    
emo2 = {'anger','frustration','jealousy','fear' ,'disgust','grief','sad','compassion','love','relief','content','awe','happy','joy','excite','postbase'};
emoorder = [1,5 3 11 9 15  13 7  10  8 14 12  2  6 4 16,17]; clear cols
for nx = 14:length(gdcomps)
    cd (['/data/common2/emotion',paths{nx},'/ersps/']);load ContDataERSPs.mat
    base = Alllongersps{1};clear emospecs tout ttestmask
    for k = 1:length(Alllongersps)
        e=emoorder(k);clear allcomps
        oneemo = Alllongersps{e}; % freqsXtimesXcomponent   
        for cmp = 1:length(gdcomps{nx})
            onecmp = oneemo(:,:,gdcomps{nx}(cmp));
            onebase = base(:,:,gdcomps{nx}(cmp));
            for ff = 1:size(onecmp,1)
                [H,tout(cmp,ff), ci,stats] = ttest2(onecmp(ff,:),onebase(ff,:));
            end;
            allcomps(cmp,:) = mean(onecmp,2)';            
        end;
        emospecs(:,:,k) = allcomps;
        ttestmask(:,:,k) = tout;
    ...
    figure;  alpha = .00001; pl = 1;
    for cp = 1:size(emospecs,1)
        subplot(length(gdcomps{nx}),17,pl)
        topoplot(EEG.icawinv(:,gdcomps{nx}(cp)),EEG.chanlocs,'plotrad',.5);pl = pl+1;
        ph=title(int2str(gdcomps{nx}(cp)));set(ph,'fontsize',7);
        x=emospecs(cp,:,:);x=squeeze(x);x=x';mask = ttestmask(cp,:,:);mask = squeeze(mask);mask = mask';
        base = repmat(x(1,:),[10 1]);
        for ee = 2:size(emospecs,3)
            subplot(length(gdcomps{nx}),17,pl)
            emo = repmat(x(ee,:),[10 1]);msk = repmat(mask(ee,:),[10 1]);
            emo = emo-base;
            emo(find(msk>alpha)) = 0;
            imagesc([1:10],freqs,emo',[-8 8]);pl = pl+1;  set(gca,'fontsize',7);
            set(gca,'ydir','norm');set(gca,'xticklabel',[]);set(gca,'ytick',[10:10:50]);
            set(gca,'yticklabel',{10  [] 30 [] 50});
            set(gca,'ticklength',[.04 .04]);
            if cp == 1
                ph= title(emo2{ee-1}); set(ph,'rotation',45);set(ph,'fontsize',7);
            end;
        end;
    end;colorbar;axcopy
    textsc(['Subject: ',int2str(nx),'    p < ',num2str(alpha)],'title');set(ph,'fontsize',13);
ALLEEG=[];EEG=[];
...
figure;cols(1,:) = [.5 .5 .5]; cols(2:16,:) = jet(15);cols(17,:) = [.5 .5 .5]; 
for cp = 1:size(emospecs,1)
    subplot(ceil(sqrt(size(emospecs,1))),round(sqrt(size(emospecs,1))),cp)
    for ee = 1:size(emospecs,3)-1
        ph = plot(freqs,emospecs(cp,:,ee),'k');hold on;
        set(ph,'color',cols(ee,:));
        if ee == 1 | ee == 17
            set(ph,'linewidth',2);
        end;
    end;
    set(gca,'xlim',[0 50]);ph = title(['Comp: ',int2str(gdcomps{nx}(cp))]);    
end;axcopy
textsc(['Subject ',int2str(nx),'   Mean spectra from each emotion'],'title');
