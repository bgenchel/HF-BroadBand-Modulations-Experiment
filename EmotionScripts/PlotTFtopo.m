...
if isempty(maxmap)
  maxmap = max(max(max(abs(mean(alldat,4)))));
end;
lcs = locs(chansubset);
figure;tftopo(alldat,times,freqs, 'timefreqs', TFois, 'chanlocs',lcs,'logfreq','on','limits',[times(1) times(end),freqs(1), freqs(end),-maxmap,maxmap],'electrodes','off');
