...
% 
% PlotCtxClustSummary(mapset,fullpath,gdcomps,clustidx,erspmask,shuffnum,fontsz,sigmask,ttl,posneg,scale,tmplorprob)
%
...
function PlotCtxClustSummary(mapset,fullpath,gdcomps,clustidx,shuffnum,fontsz,sigmask,erspmask,ctxsigs,ctxstd,ttl,scale,tmplorprob)
        ...
        for cmp = 1:length(clustidx)
            ...
            elseif  tmplorprob == 'wtdm'
                ...
                for hh = 1:length(grypnts)
                    ...
                    set(ph,'markersize',25);set(ph,'color',[.7 .7 .7]);
                end;   
                if   cmp == row | cmp == row/2
                    CnxtLabelRewFB(ctlim,gry,fontsz);
                    set(gca,'xticklabel',[]);
