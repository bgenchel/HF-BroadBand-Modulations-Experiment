...
[ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
pop_topoplot(EEG,0, [1:48] , ['as82 Emotion Components; PCA to 100 '],[] ,0, 'electrodes', 'off', 'masksurf', 'on');
set(gcf,'PaperOrientation','landscape');set(gcf,'PaperPosition',[0.25 0.25 10.5 8]);
comps = [3,5,8,10,11,13,14,17,18,19,21,27,31,34,35,40];


subj = [4,6,8,9,10,11,16,18,19,21,24,25,28,31,32,33];
figure; pl=1;  row = ceil(length(subj)/3); col = 9;
for c = 1:length(subj)
    cc=subj(c);    sbplot(row,col,pl);
    topoplot(EEG.icawinv(:,cc),EEG.chanlocs,  'electrodes', 'off', 'plotrad',0.7);
    title(int2str(cc));    sbplot(row,col,pl+1:pl+2);    
    [Pxx frq] = pwelch(EEG.icaact(cc,:),512,256,256,EEG.srate);
    plot(frq(find(frq<80)),Pxx(find(frq<80)),'b-');hold on;
    title(int2str(cc));    set(gca,'xtick',[5:5:80]);
    set(gca,'xlim',[1 80]);    set(gca,'xgrid','on');    pl=pl+3;
end;
