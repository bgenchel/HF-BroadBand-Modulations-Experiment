...
function PlotCoModDecomp(datset,datpath,plotcomps,factors,savedat,frqlim,superpos,meanvar,justone,auxpath);
    ...
    speceig = floatread([datpath,savedat,'EIGVEC.fdt'],[s.numtrials s.pcs],[],0);
    ...
    if isempty(justone)
       ...
       figure;row = round(sqrt(length(plotcomps)*2))+1; col = round(sqrt(length(plotcomps)*2))+1;pl = 1;
        ...
        if meanvar == 1
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%           
        else % regular plotting but only one factor,            
            tp = justone;
