% procedures to import emotion data on ts79
...
eeglab
rawdat = '/data/common2/emotion/ts79/ts79.bdf';
spath = '/data/common2/emotion/ts79/';
...
pack
EEG = pop_readbdf(rawdat, [5001:560:5561] ,265,[]);
...
for k = 1:length(emos)
    ...
    thresh = maxdata*.2;  % sets threshold at 10% of max value
    r=1; 
    for g = 1: size(EEG.data,2)
        if EEG.data(1,g)>thresh & EEG.data(1,g-1)<thresh
            ...
            EEG.event(end).Event_Type = 'Response'; break
        end;    
    end;
    [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);  
...
rawdat = '/data/common2/emotion/ts79/ts79.bdf';
spath = '/data/common2/emotion/ts79/';
elpfile = '/data/common2/emotion/ts79/ts79-256.elp';
ALLEEG=[];EEG=[];
%%%%  block 1   %%%%%%%%%
EEG = pop_readbdf(rawdat, [1:999:1000] ,265,[135:77:212]);
...
pack
%%%%  block 2   %%%%%%%%%
EEG = pop_readbdf(rawdat, [1001:974:1975] ,265,[135:77:212]);
...
pack
%%%%%%  block 3  %%%%%%%
EEG = pop_readbdf(rawdat, [1976:694:2670] ,265,[135:77:212]);
...
pack
%%%%%%  block 4  %%%%%%%
EEG = pop_readbdf(rawdat, [2671:969:3640] ,265,[135:77:212]);
...
pack
%%%%%%  block 5  %%%%%%%
EEG = pop_readbdf(rawdat, [3641:839:4480] ,265,[135:77:212]); % truncated cuz end of file is corruputed.
...
pack 
%%%%%%  block 6  %%%%%%%
EEG = pop_readbdf(rawdat, [4481:1039:5520] ,265,[135:77:212]); % truncated cuz end of file is corruputed.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 ts79  - ~25 yo male
data collected on August 24,2004  using 256 (265 including analog) Biosemi acquisition system
...
numframes =
      ...
      136704  +    197376  +    108288  +    162048   +   136704   +   175872


sum = 916992  
sum(1,2,4,5,6) = 808704
sum(1,2,3,4,6) = 780288
% now in linux

cat  emo1.fdt  emo2.fdt    emo4.fdt   emo5.fdt   emo6.fdt > emoall.fdt
%  
/data/common/matlab/ica_linux2.4 < /data/common2/emotion/ts79/tsemoICA.sc
...
[ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG);
sph=floatread(['/data/common2/emotion/ts79/sph',int2str(chans),'pc100.sph'],[chans chans]); 
wts=floatread(['/data/common2/emotion/ts79/wts',int2str(chans),'pc100.wts'],[100 chans]); 
EEG.icaweights=wts;EEG.icasphere=sph;EEG.icawinv=[];
[ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
pop_topoplot(EEG,0, [1:36] , 'ts79 Emotion Components; PCA to 100 ',[6 6] ,0, 'electrodes', 'off', 'masksurf', 'on');
set(gcf,'PaperOrientation','landscape');
set(gcf,'PaperPosition',[0.25 0.25 10.5 8]);
print -Pcoloring -dpsc2 [-painters]

subj = [9,11,13,14,16,18,19,21,23,24];
...
ph=textsc(['ts79 Emotion Component Spectra (PCA to 100) from ',int2str(chans),' chan data, set 1'],'title');
set(ph,'fontsize',14);
set(gcf,'PaperOrientation','landscape');
set(gcf,'PaperPosition',[0.25 0.25 10.5 8]);
...
datsets = {'emo-1-254.set','emo-2-254.set','emo-3-254.set','emo-4-254.set','emo-5-254.set','emo-6-254.set'};
spath = '/data/common2/emotion/ts79/';
